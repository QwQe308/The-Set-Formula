...
function challengeButtonText(layer, id) {
	if(options.ch){
		return (player[layer].activeChallenge==(id)?(canCompleteChallenge(layer, id)?"完成":"退出"):(hasChallenge(layer, id)?"完成":"开始"))
	}
...
function nextLayerButtonText(layer, id) {
	if(options.ch){
		return player.meta.goals.gte(tmp[layer].challenges[id].nextLayer) ? '下一层' : '未抵达目标'
	}
	return player.meta.goals.gte(tmp[layer].challenges[id].nextLayer) ? '下一层' : '未抵达目标'
}
